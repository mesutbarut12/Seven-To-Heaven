            ...
            for (i in sqlinit.einnahme().readData()) {
                ...
                if (inhalt.get(pos).id == i.id
                    ...
                    model = EinkommenModel(
                        i.summe, i.datum, i.databaseType, i.id, i.beschreibung
                    )
                    sqlinit.einnahme().deleateItem(model)
                    dataTransferPopUpDelete.data(0)
                }
            }
            inhalt.removeAt(pos)
